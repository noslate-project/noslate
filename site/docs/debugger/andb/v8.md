...
Tagging Pointer 是 V8 引擎的对象指针，类似于 c/c++ 的指针. V8 在分配对象时，会始终按照字宽对齐地址，所以使得指针的最低2位始终为零，所以被用于标识额外的信息。
其最低位为1表示是一个 Tagging Pointer，0表示是一个 Smi。次低位是 Strong / Weak 引用标识。
...
`v8 inspect` 和 `v8 object` 命令行的区别在于，参数是否为一个 Tagging Pointer 或者是 对象的内存地址。
