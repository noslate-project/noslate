...
### 确定依赖镜像版本
在[版本历史](noslate/release_notes.md)中确认自己要使用的版本号，并在 Dockerfile 中引入：
```
ARG NOSLATE_VERSION
...
RUN mkdir -p $NOSLATE_LOGDIR
```
+ 如需了解更多关于日志处理的内容，可以参考[日志处理](noslate/tutorials/logging.md)。
+ 如需了解更多环境变量的内容，可以参考[环境变量](noslate/references/envionment_variables.md)
### 集成 Gateway 组件
+ [Gateway 组件介绍](noslate/references/gateway.md)
+ [如何自定义 Gateway 组件](noslate/tutorials/custom_gateway.md)

Gateway 组件的细节可以参考上面提供的两篇文档。在这里我们使用已经写好的 [Demo Gateway](https://github.com/noslate-project/noslate/tree/main/distro/resources/gateway)，将它添加到镜像中。
```
...
```
这里提供一份写好的 [start.sh](https://github.com/noslate-project/noslate/tree/main/distro/resources/start.sh) 用作参考。
